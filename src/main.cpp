...
int main(int argc, char **argv) {
  ...
  oak.start(
      [&](const OakFrame &f) {
        ...
        if (!f.left_gray_u8.empty()) {
          logger.write_packet(
              PacketType::OakLeftGrayU8, f.left_gray_u8.data,
        ...
        if (!f.disparity_u16.empty()) {
          logger.write_packet(
              PacketType::OakDisparityu16, f.disparity_u16.data,
        ...
        logger.write_packet(PacketType::OakQuality, &q, (uint32_t)sizeof(q),
                            f.host_t_ns);
      },
      640, 400, 30);
#else
  ...
  const bool ok = px.start_serial(serial, baud, [&](const ImuSample &s) {
    ...
    logger.write_packet(PacketType::PixhawkIMU, &p, (uint32_t)sizeof(p),
                        s.host_t_ns);
  ...
  if (!ok) {
    logger.write_text_line(
        "Pixhawk connect failed. Check serial path and permissions.");
  } else {
