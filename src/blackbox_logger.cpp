...
namespace naina {
    ...
    BlackBoxLogger::BlackBoxLogger(const std::string& session_dir):  // session_dir is a string containing a folder path
    session_dir_(session_dir) { // session_dir_ (a class variable) is set to session_dir
        fs::create_directories(session_dir_); // create the directory if it does not already exist
    ...
    BlackBoxLogger::~BlackBoxLogger() {
        std::lock_guard<std::mutex> lock(mtx_); // lock mutex so no other thread can use the files
        if (bin_.is_open()) bin_.close(); // close the binary file
        if (txt_.is_open()) txt_.close(); // close the text file
    }
    ...
    uint64_t BlackBoxLogger::next_seq(PacketType type) {
        const uint16_t key = static_cast<uint16_t>(type); // convert the enum PacketType into a 16-bit integer
    ...
    void BlackBoxLogger::write_packet(PacketType type, const void* payload, uint32_t payload_bytes, uint64_t t_ns) {
        std::lock_guard<std::mutex> lock(mtx_); // lock mutex so no other thread can use the file

        PacketHeader hdr{}; // create a PakcetHeader object and zero-initialize all fields
        hdr.magic = kMagic;
        ...
        bin_.write(reinterpret_cast<const char*>(&hdr), sizeof(PacketHeader)); // write the header as raw bytes to the binary file
        if (payload_bytes > 0 && payload != nullptr) { // if there is payload data
            bin_.write(reinterpret_cast<const char*>(payload), payload_bytes); // write that as well
        }
    }
