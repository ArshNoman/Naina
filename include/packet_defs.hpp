...
namespace naina {
    ...
    enum class PacketType : uint8_t {
        ...
        PixhawkIMU = 10, // raw IMU sensor data (linear acceleration, angular velocity, timestamps)
        PixhawkStatus = 11 // system status information from Pixhawk (arm state, error codes, health flags)
    };
    ...
    struct OAKpayload {
        float depth_valid_ratio; // ratio (0-1) of valid depth measurements to total depth pixels
        float mean_disparity; // average disparty value measured over the disparity map
        float exposure_time_us; // sensor exposure duration in microseconds
        float analog_gain; // how much the camera amplified the signal to make the image brighter
        uint32_t width;
        uint32_t height;
    };
    ...
    #pragma pack(pop) // restore normal memory alignment rules (reverts line 17's command)
