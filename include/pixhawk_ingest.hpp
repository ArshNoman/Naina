...
struct ImuSample {
  ...
  float gyro_rad_s[3];  // angular velocity around x/y/z axes, stored as a
                        // fixed-size C-style array
  float accel_m_s2[3];  // linear acceleration along x/y/z axes, also a
                        // fixed-size array
};

// callback type alias
using ImuCallback =
    std::function<void(const ImuSample &)>; // structure of any function taking
                                            // in ImuSample and outputting void
// the callback function (cb) determines the handling of each sample data
// (logging, odometry, telemetry) having a callback functione essentially
// separates the implementations of data acquisitions and data processing
...
public:
  ...
  bool start_serial(const std::string &serial_path, int baudrate,
